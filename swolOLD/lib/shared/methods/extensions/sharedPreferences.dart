...
class SharedPrefsExt {
  ...
  static ValueNotifier getTermAgreed() =>
      _nameToValueNotifier[ValueName.TermsAgreed];
  static ValueNotifier getInitialControlsShown() =>
      _nameToValueNotifier[ValueName.InitialControlsShown];
  static ValueNotifier getIntroductionShown() =>
      _nameToValueNotifier[ValueName.IntroductionShown];
  static ValueNotifier getSaveShown() =>
      _nameToValueNotifier[ValueName.SaveShown];
  static ValueNotifier getSearchShown() =>
      _nameToValueNotifier[ValueName.SearchShown];
  static ValueNotifier getNotificationRequested() =>
      _nameToValueNotifier[ValueName.NotificationRequested];

  //setters (must update local and global)
  static setIsDark(bool isDark) => _setLocalGlobal(ValueName.IsDark, isDark);
  static setTermsAgreed(bool termsAgreed) =>
      _setLocalGlobal(ValueName.TermsAgreed, termsAgreed);
  static setInitialControlsShown(bool initialControlsShown) =>
      _setLocalGlobal(ValueName.InitialControlsShown, initialControlsShown);
  static setIntroductionShown(bool introductionShown) =>
      _setLocalGlobal(ValueName.IntroductionShown, introductionShown);
  static setSaveShown(bool saveShown) =>
      _setLocalGlobal(ValueName.SaveShown, saveShown);
  static setSearchShown(bool searchShown) =>
      _setLocalGlobal(ValueName.SearchShown, searchShown);
  static setNotificationRequested(bool notificationRequested) =>
      _setLocalGlobal(ValueName.NotificationRequested, notificationRequested);

  //-------------------------private-------------------------

  static Future _getOrSetToDefault(ValueName name) async {
    bool isBool = true;
  ...
  static _setLocalGlobal(
    ...
    //update globally
    await _set(isBool, name.toString(), value);
  }

  static dynamic _get(bool isBool, String key) {
    if (_savedPreferences.containsKey(key)) {
      if (isBool)
        return _savedPreferences.getBool(key);
      else
        return _savedPreferences.getInt(key);
    }
  }

  static Future<dynamic> _set(bool isBool, String key, dynamic value) async {
    //start the process of saving in shared preferences asynchronously
    if (isBool)
      await _savedPreferences.setBool(key, value);
    else
      await _savedPreferences.setInt(key, value);
